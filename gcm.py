...
# Python module gcm.py
# Autogenerated from gcm.ui
...
def initialise_encyption_key():
    ...
    enc_passwd = "%x" % (x*y)
    try:
        with os.fdopen(os.open(KEY_FILE, os.O_WRONLY | os.O_CREAT, 0x0600), 'w') as f:
            f.write(enc_passwd)
    except:
...
class Wmain(SimpleGladeApp):
                   ...
                   widget.reset(True, True)
                    ...
                    if not hasattr(widget, "command"):
                        ...
                        widget.spawn_sync(
                            Vte.PtyFlags.DEFAULT,
                            os.environ['HOME'],
                            [widget.command[0], widget.command[1]],
                            [],
                            GLib.SpawnFlags.DO_NOT_REAP_CHILD,
                            None,
                            None,
    ...
    def addTab(self, notebook, host):
        #try:
            ...
            #if v.emulation != os.getenv("TERM"):
            #    os.environ['TERM'] = v.emulation
            ...
            if len(fcolor)>0 and len(bcolor)>0:
                v.set_colors(
                    Gdk.RGBA.from_color(Gdk.color_parse(fcolor)),
                    Gdk.RGBA.from_color(Gdk.color_parse(bcolor)),
                    []
                )
            ...
            if conf.TRANSPARENCY > 0:
                if not self.real_transparency:
                    ...
                    if len(bcolor)>0:
                        v.set_background_tint_color(Gdk.Color(bcolor))
                else:
    ...
    def on_txtSearch_focus(self, widget, *args):
        if widget.get_text() == _('buscar...'):
            widget.modify_text(Gtk.StateType.NORMAL, Gdk.Color('black'))
            widget.set_text('')
    #-- Wmain.on_txtSearch_focus }
    ...
    def on_txtSearch_focus_out_event(self, widget, *args):
        if widget.get_text() == '':
            widget.modify_text(Gtk.StateType.NORMAL, Gdk.Color('darkgray'))
...
class Whost(SimpleGladeApp):
    ...
    def init(self, group, host=None):
        ...
        self.btnFColor.set_color(Gdk.Color(fcolor))
        self.btnBColor.set_color(Gdk.Color(bcolor))
...
class Wconfig(SimpleGladeApp):
    ...
    def new(self):
        ...
        slist = sorted(shortcuts.iteritems(), key=lambda xy: (xy[1],xy[0]))
